...
export const register = async (req, res) => {
  ...
  try {
    const { rows: data } = await pool.query(q, [req.body.email, req.body.username]);
    ...
    const insertQ = "INSERT INTO \"user\" (username, email, password) VALUES ($1, $2, $3)";
...
export const login = async (req, res) => {
  ...
  try {
    const { rows: data } = await pool.query(q, [req.body.username]);
